...
async function getData(){
        ...
        const noOfPages=Math.ceil(result.length/10)
        console.log(noOfPages)

        
        const bttn_F=document.createElement("button")
        bttn_F.innerText="First";
        pagination.append(bttn_F)
        bttn_F.onclick=()=>{
            document.querySelector(".dsply").innerHTML=""
            const homePage=result.slice(0,10)
        ...
        const bttn_P=document.createElement("button")
        bttn_P.innerText="Previous";
        pagination.append(bttn_P)
        bttn_P.onclick=()=>{
            if(temp>1){
                temp=temp-1

                document.querySelector(".dsply").innerHTML=""
                const showPage=result.slice((temp-1)*10,((temp)*10))
                console.log(temp,"checking temp value")          
        ...
        for(let i=1;i<=noOfPages;i++){
                ...
                bttn.onclick=()=>{
                    ...
                    const showPage=result.slice((i-1)*10,(i*10))
                    temp=i;
                    document.querySelector(".dsply").innerHTML=""
                    //document.querySelector()
                    showPage.forEach(element => {
        ...
        const bttn_N=document.createElement("button")
        bttn_N.innerText="Next";
        pagination.append(bttn_N)
        bttn_N.onclick=()=>{
            if(temp<noOfPages){
                temp=temp+1
        ...
        const bttn_L=document.createElement("button")
        bttn_L.innerText="Last";
        pagination.append(bttn_L)
        bttn_L.onclick=()=>{
            temp=10;
            document.querySelector(".dsply").innerHTML=""
            const homePage=result.slice(result.length-10,result.length)
            homePage.forEach(element => {
...
async function display(x){
   const date=new Date(x.released).toDateString();
    //console.log(x)
    document.querySelector(".dsply").innerHTML +=`<div class="page">
    <div class="content">
        <div class="detail"><span class="title">ID:</span> ${x.id}</div>
        <div class="detail"><span class="title">Name:</span> ${x.name}</div>
        <div class="detail"><span class="title">Email Id:</span>${x.email}</div>
    </div>
